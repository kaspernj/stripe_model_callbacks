...
describe "payment intents - succeeded" do
  ...
  describe "#execute!" do
    it "updates the payment intent and logs it" do
      expect { mock_stripe_event("payment_intent.succeeded") }
        .to change(StripePaymentIntent, :count).by(0)
        .and change(ActiveRecordAuditable::Audit.where_type("StripePaymentIntent").where_action("succeeded"), :count).by(1)
