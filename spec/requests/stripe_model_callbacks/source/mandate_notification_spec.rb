...
describe "source mandate notification" do
  ...
  let(:payload) { File.read("spec/fixtures/stripe_events/source/source.mandate_notification.json") }
  ...
  describe "#execute!" do
    it "creates an activity and updates the source" do
      ...
      expect(source.created).to eq Time.zone.parse("2018-02-06 07:43:35")
      expect(source.livemode).to eq false
      expect(source.client_secret).to eq "src_client_secret_CGz5Ss7pgMg0MVqljzgoEEUs"
