...
describe "payment intents - processing" do
  ...
  describe "#execute!" do
    it "updates the payment intent and logs it" do
      expect { PublicActivity.with_tracking { mock_stripe_event("payment_intent.processing") } }
        .to change(StripePaymentIntent, :count).by(0)
        .and change(PublicActivity::Activity.where(key: "stripe_payment_intent.processing"), :count).by(1)
