...
describe "plan updated" do
  ...
  let(:payload) { File.read("spec/fixtures/stripe_events/plan_created.json") }
  before { bypass_event_signature(payload) }

  describe "#execute!" do
    it "marks the charge as refunded" do
      expect { post "/stripe-events", params: payload }
        .to change(StripeModelCallbacks::StripePlan, :count).by(1)

      plan = StripeModelCallbacks::StripePlan.last

      expect(response.code).to eq "200"

      expect(plan.identifier).to eq "gold_00000000000000"
      expect(plan.amount.format).to eq "$20.00"
      expect(plan.created_at).to eq Time.zone.parse("2018-02-05 15:52:25")
