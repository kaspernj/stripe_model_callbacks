...
describe "payment methods - updated" do
  ...
  describe "#execute!" do
    it "creates the subscription" do
      expect { PublicActivity.with_tracking { mock_stripe_event("payment_method.updated") } }
        .to change(StripePaymentMethod, :count).by(0)
        .and change(PublicActivity::Activity.where(key: "stripe_payment_method.updated"), :count).by(1)
