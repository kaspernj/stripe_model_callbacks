...
describe "sku created" do
  ...
  let(:payload) { File.read("spec/fixtures/stripe_events/sku_created.json") }
  before { bypass_event_signature(payload) }

  describe "#execute!" do
    it "creates the subscription" do
      expect { post "/stripe-events", params: payload }
        .to change(StripeModelCallbacks::StripeSku, :count).by(1)

      created_sku = StripeModelCallbacks::StripeSku.last
