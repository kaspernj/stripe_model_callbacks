...
describe "payment intents - amount capturable updated" do
  ...
  describe "#execute!" do
    it "updates the payment intent and logs it" do
      expect { PublicActivity.with_tracking { mock_stripe_event("payment_intent.amount_capturable_updated") } }
        .to change(StripePaymentIntent, :count).by(0)
        .and change(PublicActivity::Activity.where(key: "stripe_payment_intent.amount_capturable_updated"), :count).by(1)
