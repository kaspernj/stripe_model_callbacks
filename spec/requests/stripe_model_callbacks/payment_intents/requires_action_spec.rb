...
describe "payment intents - requires action" do
  ...
  describe "#execute!" do
    it "updates the payment intent and logs it" do
      expect { PublicActivity.with_tracking { mock_stripe_event("payment_intent.requires_action") } }
        .to change(StripePaymentIntent, :count).by(0)
        .and change(PublicActivity::Activity.where(key: "stripe_payment_intent.requires_action"), :count).by(1)
